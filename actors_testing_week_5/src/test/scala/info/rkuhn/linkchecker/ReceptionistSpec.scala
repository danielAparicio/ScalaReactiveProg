  ...
  class FakeController extends Actor {
    ...
    def receive = {
      case Controller.Check(url, depth) =>
        //we should use this instead of Thread sleep , that is blocking actor should never block/sleep
...
class ReceptionistSpec extends TestKit(ActorSystem("ReceptionistSpec"))
  ...
  "A Receptionist" must {
    ...
    "reply with a result" in {
      //we dont need an step father as the receptionist is the main parent
