...
class Getter(url: String, depth: Int) extends Actor {
  ...
  //we need this context as get needs an EXECUTOR and pipe needs an EXECUTION CONTEXT
  implicit val executor = context.dispatcher.asInstanceOf[Executor with ExecutionContext]
  def client: WebClient = AsyncWebClient

  //we do this because get is returning a future , the pipteTo is a pattern that is sending back to self
  //a message with the response
