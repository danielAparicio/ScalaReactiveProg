...
class BinaryTreeSet extends Actor with Stash {
  ...
  def garbageCollecting(newRoot: ActorRef): Receive = {
    case message: Operation => stash() //AKKA stash
    case CopyFinished => {
      root ! PoisonPill
      root = newRoot
...
class BinaryTreeNode(val elem: Int, initiallyRemoved: Boolean) extends Actor {
  ...
  //loop through branches to the subtree the element should be in
  def checkBranch(op: Operation)(lastTree: Position => Unit) {
  ...
  //check the branch
  def branch(elem: Int): Position = if (this.elem == elem) Center else if (this.elem < elem) Right else Left   
  ...
  val normal: Receive = {
    case op @ Insert(requester, id, elem) => checkBranch(op) {
      pos => pos match {
        case Center => removed = false
        case _ => subtrees += pos -> context.actorOf(BinaryTreeNode.props(elem, initiallyRemoved = false))
      }
      requester ! OperationFinished(id)  
    } 
    case op @ Contains(requester, id, elem) => checkBranch(op) {
      case Center => requester ! ContainsResult(id, !removed)
      case _ => requester ! ContainsResult(id, false)
    } 
    case op @ Remove(requester, id, elem) => checkBranch(op) { 
      pos => {
        if (pos == Center) removed = true
        requester ! OperationFinished(id)
      }
    }
    case CopyTo(newRoot) => {
      if (!removed) newRoot ! Insert(self, elem, elem) //element as id , they should be DIFERENT
